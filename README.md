...
**"기존 데이터 활용(농림수산식품교육문화정보원 Data)"**과 **"신규 데이터 적재(농림수산식품교육문화정보원 Data에 없는 식품 정보)"**의 프로세스를 명확히 분리하여 설계했습니다.
...
* **해결 (Solution):**
    ...
    2. **스키마 최적화:** DB 설계 시 `음식명`이 아닌 `식품코드`를 **Primary Key(기본키)**로 설정하여 데이터의 유일성을 보장했습니다.
...
이 프로젝트는 단순히 웹 서비스를 만드는 것을 넘어, **"Raw Data(CSV)가 어떻게 가치 있는 Database로 변환되는가"**를 경험한 엔지니어링 프로젝트였습니다.
